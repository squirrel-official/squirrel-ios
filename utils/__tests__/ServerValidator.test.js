...
describe('ServerValidator', () => {
	...
	describe('validateServer()', () => {
		...
		it('should return valid if product name is "squirrel Server"', async () => {
			fetch.mockResponse(JSON.stringify({ ProductName: 'squirrel Server' }));

			const result = await validateServer({ url: new URL('https://foobar/') });
			expect(result.isValid).toBe(true);
		});

		it('should return invalidProduct message if product name is not "squirrel Server"', async () => {
